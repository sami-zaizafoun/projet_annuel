...
class Cell:
    ...
    def draw(self, draw):
        ...
        #bottom wall
        if self.walls & 1 == 1:
            draw.line([(margin + cellSize * self.x, margin + cellSize * self.y + cellSize), (margin + cellSize * self.x + cellSize, margin + cellSize * self.y + cellSize)], fill="blue", width=3)
        #right wall
        if (self.walls >> 1 & 1) == 1:
            draw.line([(margin + cellSize * self.x + cellSize, margin + cellSize * self.y), (margin + cellSize * self.x + cellSize, margin + cellSize * self.y + cellSize)], fill="blue", width=3)
        #top wall
        if (self.walls >> 2 & 1) == 1:
            draw.line([(margin + cellSize * self.x, margin + cellSize * self.y), (margin + cellSize * self.x + cellSize, margin + cellSize * self.y)], fill="blue", width=3)
        #left wall
        if (self.walls >> 3 & 1) == 1:
            draw.line([(margin + cellSize * self.x, margin + cellSize * self.y), (margin + cellSize * self.x, margin + cellSize * self.y + cellSize)], fill="blue", width=3)

        #start point
        if self.x == start[0] and self.y == start[1]:
            draw.line((round(margin + cellSize * self.x + cellSize / 4), round(margin + cellSize * self.y + cellSize / 4)))
        #end point
        ...
        #inbetween
            ...
            draw.ellipse([margin + cellSize * self.x + cellSize / 6, margin + cellSize * self.y + cellSize / 6, margin + cellSize * self.x + cellSize / 6 + cellSize / 6, margin + cellSize * self.y + cellSize / 6 + cellSize / 6], fill="yellow")
...
def main():
    ...
    draw.rectangle([margin, margin, margin + drawingWidth, margin + drawingHeight], fill="black")
    ...
    start = (round(rows/2), round(cols/2))
